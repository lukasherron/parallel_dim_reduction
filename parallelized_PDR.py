...
def PDR_algo(path_to_data, nK, nL, alph, samp_freq, stop_search, max_iter):

        # LOADING DATA
        loader = DataLoader()
        xs, metx, labels = loader.load_data(path_to_data)
        idx_train, idx_test, idx_val = loader.split_data(test_size=0.2, val_size=0.1)
        ...
        pointer_to_model = "/blue/pdixit/lukasherron/parallel_dim_reduction/data/models/nK=" + str(nK) + "/nL=" + str(nL) + "/"
        filename_model = "model_nK=" + str(nK) + "_nL=" + str(nL) + "_alph=" + str(alph) + "_.xlsx"
        ...
        # TESTING PHASE
        pointer_to_pred = "/blue/pdixit/lukasherron/parallel_dim_reduction/data/predictions/nK=" + str(nK) + "/nL=" + str(nL) + "/"
        filename_pred = "model_nK=" + str(nK) + "_nL=" + str(nL) + "_alph=" + str(alph) + "_pred.xlsx"
        ...
        plt.xlim(-5, 0)
        plt.ylim(-5, 0)
        pointer_to_fig = "/blue/pdixit/lukasherron/parallel_dim_reduction/plots_and_figs/model_specific/nK=" + str(nK) + "/nL=" + str(nL) + "/"
...
def PDR_alph_range(path_to_data, nK, nL, alph_arr, samp_freq, stop_search, max_iter):
    for alph in alph_arr:
        alph = float('%.3f' % alph)
        PDR_algo(path_to_data, nK, nL, alph, samp_freq, stop_search, max_iter)

alph_max = 1
alph_div = 0.01
alph_arr = np.arange(0, int(alph_max/alph_div) + 1) * alph_div
path_to_data = "/blue/pdixit/lukasherron/parallel_dim_reduction/data/datasets/combined_data.mat"

num_L_components = 5
num_K_components = 28

PDR_alph_range(path_to_data, num_K_components, num_L_components, alph_arr, 100, 1000, 20000)
